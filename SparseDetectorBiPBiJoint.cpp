...
void Labeler::extractLinearFeatures(vector<string>& features, const Instance* pInstance) {
  ...
  vector<set<string> > uniBowSet, biBowSet;
  uniBowSet.resize(segStyleSize);
  biBowSet.resize(segStyleSize);
  string feat = "";
  for (int i = 0; i < segStyleSize; i++) {
    int curInstSize = words[i].size();
    for (int j = 0; j < curInstSize; ++j) {
  ...
  for (int i = 0; i < segStyleSize; i++) {
    for (int j = i + 1; j < segStyleSize; j++) {
      for (it = uniBowSet[i].begin(); it != uniBowSet[i].end(); it++) {
        if (uniBowSet[j].find(*it) != uniBowSet[j].end()) {
      ...
      for (it = biBowSet[i].begin(); it != biBowSet[i].end(); it++) {
        if (biBowSet[j].find(*it) != biBowSet[j].end()) {
          vector<string> vecInfo;
          split_bystr(*it, vecInfo, "#]");
