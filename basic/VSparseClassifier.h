...
public:
  ...
  inline dtype process(const vector<vector<Example> >& examples, int iter) {
    _eval.reset();
    dtype cost = 0.0;
    int offset = 0;
    ...
    for (int count = 0; count < example_num; count++) {
      const vector<Example> & currexample = examples[count];
      int segStyleSize = currexample.size();
      for (int i = 0; i < segStyleSize; i++) {
        Example example = currexample[i];
  ...
  void checkgrads(const vector<vector<Example> >& examples, int iter) {
    for(int i = 0; i<examples.size();i++){
      checkgrad(this, examples[i], _layer_linear._W, _layer_linear._gradW, "_layer_linear._W", iter, _layer_linear._indexers, false);
      checkgrad(this, examples[i], _layer_linear._b, _layer_linear._gradb, "_layer_linear._b", iter);
    }
  }
