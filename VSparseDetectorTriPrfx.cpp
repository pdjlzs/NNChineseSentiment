...
void Labeler::extractLinearFeatures(vector<vector<string> >& multGrpfeatures, const Instance* pInstance) {
  multGrpfeatures.clear();
  const vector<vector<vector<string> > > &words = pInstance->words;
  int segStyleSize = words.size();
  if (segStyleSize > 4) {
  ...
  for (int i = 0; i < segStyleSize; i++) {
    ...
    for (int j = 0; j < curInstSize; ++j) {
      ...
      for (int k = 0; k < curWordSize; k++) {
        feat = m_labelAlphabet.from_id(i) + "F1U=" + curr_words[k];
        features.push_back(feat);
        string prevword = k - 1 >= 0 ? curr_words[k - 1] : nullkey;
        feat = m_labelAlphabet.from_id(i) + "F2B=" + prevword + seperateKey + curr_words[k];
        features.push_back(feat);
        string prev2word = k - 2 >= 0 ? curr_words[k - 2] : nullkey;
        feat = m_labelAlphabet.from_id(i) + "F3T=" + prev2word + seperateKey + prevword + seperateKey + curr_words[k];
        features.push_back(feat);

      }
    }
    multGrpfeatures.push_back(features);
  }
...
void Labeler::convert2Example(const Instance* pInstance, vector<Example>& currtExam) {
  currtExam.clear();
  const string &label = pInstance->label;
  const vector<vector<vector<string> > > &words = pInstance->words;
  //const vector<vector<vector<string> > > &chars = pInstance->chars;
  ...
  for (int i = 0; i < segStyleSize; i++) {
    ...
    currtExam.push_back(exam);
  }

  if (m_linearfeat > 0) {
    vector<vector<string> > linear_features;
    extractLinearFeatures(linear_features, pInstance);

    for (int i = 0; i < linear_features.size(); i++) {
      for (int j = 0; j < linear_features[i].size(); j++) {
        int curFeatId = m_featAlphabet.from_string(linear_features[i][j]);
        if (curFeatId >= 0)
          currtExam[i].m_linears.push_back(curFeatId);
      }
    }
...
void Labeler::initialExamples(const vector<Instance>& vecInsts, vector<vector<Example> >& vecExams) {
  int numInstance;
  for (numInstance = 0; numInstance < vecInsts.size(); numInstance++) {
    const Instance *pInstance = &vecInsts[numInstance];
    vector<Example> curExam;
  ...
  /*debug
   std::cout<<std::endl<<vecExams.size()<<std::endl;
   for (int i = 0; i < vecExams.size(); i++) {
   std::cout<<vecExams[i].size()<<std::endl;
   for (int j = 0; j < vecExams[i].size(); j++) {
   for (int idx = 0; idx < vecExams[i][j].m_labels.size(); idx++)
   std::cout << "label: " << vecExams[i][j].m_labels[idx] << " ";
   std::cout << "feat: ";
   for (int k = 0; k < vecExams[i][j].m_linears.size(); k++) {
   std::cout << vecExams[i][j].m_linears[k] << " ";
   }
   std::cout << std::endl;
   }
   std::cout << std::endl;
...
void Labeler::train(const string& trainFile, const string& devFile, const string& testFile, const string& modelFile, const string& optionFile,
  ...
  vector<vector<Example> > trainExamples, devExamples, testExamples;
  ...
  static vector<vector<Example> > subExamples;
  ...
  for (int iter = 0; iter < m_options.maxIter; ++iter) {
    ...
    if (devNum > 0) {
      ...
      for (int idx = 0; idx < devExamples.size(); idx++) {
        ...
        string result_label;
        dtype confidence;
        int segStyleSize = devExamples[idx].size();
        for (int i = 0; i < segStyleSize; i++) {
          string label;
          confidence = predict(devExamples[idx][i].m_linears, devExamples[idx][i].m_features, label);
          result_vote += atoi(label.c_str());
        }
        if (result_vote == 0)
          result_label = "0";
        else if (result_vote < 0)
          result_label = "-1";
        else
          result_label = "1";
      ...
      if (testNum > 0) {
        ...
        for (int idx = 0; idx < testExamples.size(); idx++) {
          ...
          string result_label;
          dtype confidence;
          int segStyleSize = testExamples[idx].size();
          for (int i = 0; i < segStyleSize; i++) {
            string label;
            dtype confidence = predict(testExamples[idx][i].m_linears, testExamples[idx][i].m_features, label);
            result_vote += atoi(label.c_str());
            //std::cout << label << "  " << result_vote << std::endl;
          }
          if (result_vote == 0)
            result_label = "0";
          else if (result_vote < 0)
            ...
            result_label = "1";
          //std::cout << "result_label: " << result_label << std::endl;
